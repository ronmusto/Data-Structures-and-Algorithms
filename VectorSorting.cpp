...
int partition(vector<Bid>& bids, int begin, int end) {
	...
	while (!done) {
		...
		if (low >= high) {
			...
			// swap the low and high bids using built in method
...
void selectionSort(vector<Bid>& bids) {
	// index to current minimum bid
	int min;
	//pos is the position in the bids that marks sorted/unsorted
	for (unsigned pos = 0; pos < bids.size(); ++pos) {
		min = pos;
        // iterating through array looking for smaller value and saving it
		for (unsigned j = pos + 1; j < bids.size(); ++j) {
			if (bids.at(j).title.compare(bids.at(min).title) < 0) {
				min = j;
			}
		}
        // swap min bid with current position
		if (min != pos) {
